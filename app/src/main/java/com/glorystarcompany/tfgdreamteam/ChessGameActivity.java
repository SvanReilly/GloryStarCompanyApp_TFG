...
import java.util.ArrayList;
        ...
        public class ChessGameActivity extends AppCompatActivity implements View.OnClickListener {
            ...
            private UsuariosBD updateBDForUsuario;
            ...
            protected void onCreate(Bundle savedInstanceState) {
                ...
                usuarioActualChessGameActivity = ChessActivityContainerToGames.getStringExtra("usuarioActualFirstPageActivity");

                updateBDForUsuario = new UsuariosBD(getApplicationContext());

                //Codigo de la ejecucion del juego

                initializeBoard();

                game_over = (ImageView)findViewById(R.id.game_over);
                Glide.with(this).asGif().load(R.drawable.gameover).into(game_over);
                        ...
                        if(moveIsAllowed(listOfCoordinates , clickedPosition)){
                            ...
                            if(Board[clickedPosition.getX()][clickedPosition.getY()].getPiece() instanceof King){
                                if(Board[clickedPosition.getX()][clickedPosition.getY()].getPiece().isWhite() != FirstPlayerTurn){
                                    ...
                                    //Guardar resultado respectivo en la bbdd para el jugador loggeado.


                                    updateBDForUsuario.updateDefeatsUsuario(usuarioActualChessGameActivity);

                                    // Volver al ActivityThird.

                                    Intent goBackToThirdActivityAfterPlaying = new Intent(this, ThirdActivity.class);
                                    goBackToThirdActivityAfterPlaying.putExtra("usuarioResultante", usuarioActualChessGameActivity);
                                    startActivity(goBackToThirdActivityAfterPlaying);
                                    finish();
                                }
                            }
                                    ...
                                    if(moveIsAllowed(listOfCoordinates , clickedPosition)){
                                        ...
                                        if(Board[clickedPosition.getX()][clickedPosition.getY()].getPiece() instanceof King){
                                            if(Board[clickedPosition.getX()][clickedPosition.getY()].getPiece().isWhite() != FirstPlayerTurn){
                                                ...
                                                updateBDForUsuario.updateWinsUsuario(usuarioActualChessGameActivity);
